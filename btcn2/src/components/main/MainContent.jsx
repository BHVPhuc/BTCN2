...
export default function MainContent() {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        const popularData = await movieService.getMostPopular({
          page: 1,
          limit: 12,
        });
        const popularData2 = await movieService.getMostPopular({
          page: 2,
          limit: 3,
        });
        ...
        const topRatedData2 = await movieService.getTopRated({
          page: 2,
          limit: 3,
          category: "IMDB_TOP_50",
        });
        const featuredData = await movieService.getMostPopular({
          page: 1,
          limit: 5,
        });
