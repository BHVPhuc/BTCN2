...
export const movieService = {
  async getMostPopular({page = 1, limit = 12} = {}) {
    const res = await fetch(`${API_BASE_URL}/movies/most-popular?page=${page}&limit=${limit}`, {
      headers,
    });
  ...
  async getTopRated({page = 1, limit = 12, category = "IMDB_TOP_50"} = {}) {
    const res = await fetch(`${API_BASE_URL}/movies/top-rated?category=${category}&page=${page}&limit=${limit}`, {
      headers,
    });
  ...
  async getAll({page = 1, limit = 10}) {
    const res = await fetch(`${API_BASE_URL}/movies?page=${page}&limit=${limit}`, { headers });
